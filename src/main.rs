...
async fn main() {
    ...
    let ohlcv = wrappers::get_ohlc(&eod_client, "AAPL", "US", "2023-10-01", "2023-11-20").await.unwrap();
    // println!("{:#?}", &ohlcv);
    ...
    let records = database_service::read_timeseries(&client, "2023-10-04", "2023-11-01", metadata,"equity", "spot", "1d", database_service::OhlcGranularity::Hours).await;
